...
// PDF.js needs to be loaded via createRequire because the legacy build
// expects CommonJS-style imports. The legacy build runs without a worker.
...
export async function extractPdfText(pdfBytes: Uint8Array): Promise<ExtractionResult> {
  // Use the legacy build to avoid needing a worker in Node.
  const pdfjs: PdfJsLib = require("pdfjs-dist/legacy/build/pdf.mjs");
